...
# --- Версия приложения ---
APP_VERSION = "1.3.9.3" # Исправление TypeError при multipart, возврат промежуточного вебхука
...
async def upload_file_via_api(
    ...
    http_headers = { # Основные заголовки, Content-Type будет установлен httpx для multipart
...
async def process_file_and_send_webhook(
    ...
    # --- Отправка промежуточного вебхука о начале обработки (если это первая попытка) ---
    if _client_acquire_attempt == 1:
        ...
        db_tasks_for_initial_hook = await load_webhook_tasks_db()
        task_info_for_initial_hook = db_tasks_for_initial_hook.get(task_id)
        if task_info_for_initial_hook and task_info_for_initial_hook.get("webhook_url"):
            initial_payload = {
                "task_id": task_id,
                "original_url": original_url,
                "timestamp": datetime.datetime.now(pytz.utc).isoformat(),
                "status": "processing_started", # Или "accepted", "queued"
                "message": "Задача принята в обработку."
            }
            if client_metadata: initial_payload["metadata"] = client_metadata
            await _send_intermediate_webhook(task_id, task_info_for_initial_hook["webhook_url"], initial_payload, task_logger)
    else:
    ...
    # --- Конец отправки промежуточного вебхука ---
    ...
    try:
        ...
        if task_info_for_processing and task_info_for_processing.get("status") != "processing": # Может быть уже "processing", если это рестарт
...
async def process_link_with_webhook_v2(
    ...
    # Сразу после добавления задачи в очередь (и перед фактическим началом ее обработки в фоне),
    # можно отправить первоначальный вебхук "принято в работу", если это требуется.
    # Однако, если process_file_and_send_webhook сама отправляет такой вебхук в начале,
    # то здесь это делать не нужно, чтобы избежать дублирования.
    # В текущей версии process_file_and_send_webhook отправляет такой вебхук.
