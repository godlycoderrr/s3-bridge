...
from urllib.parse import unquote as url_unquote, urlparse, quote as url_quote, urljoin
...
# --- Версия приложения ---
APP_VERSION = "1.3.9.2" # Изменение способа загрузки на multipart/form-data
...
# Имя поля формы, которое ожидает ваш API для файла (если multipart)
# Если API не требует конкретного имени, 'file' часто используется по умолчанию.
# Уточните у разработчиков API, если это не работает.
UPLOAD_API_FORM_FIELD_NAME = os.getenv("UPLOAD_API_FORM_FIELD_NAME", "file")
...
if not S3_PUBLIC_BASE_URL and UPLOAD_API_ENDPOINT: 
    logger.warning("S3_PUBLIC_BASE_URL не задан в .env. URL файлов в вебхуках могут быть относительными, если API загрузки возвращает относительные пути.")
...
async def upload_file_via_api(
    file_path: str, request_id: str, s3_filename_for_log: str, original_human_readable_filename: str
) -> Optional[str]:
    req_logger = RequestIdAdapter(base_logger, {'request_id': request_id})
    ...
    req_logger.info(f"🚀 Подготовка к загрузке (multipart): '{original_human_readable_filename}' (S3 имя для логов: '{s3_filename_for_log}') из '{file_path}'")
    file_size = os.path.getsize(file_path)
    req_logger.info(f"📦 Размер: {file_size} байт")
    
    # Для multipart/form-data Content-Type файла определяется в самой части,
    # а Content-Type всего запроса будет multipart/form-data; boundary=...
    # httpx устанавливает это автоматически при использовании параметра `files`.
    # Нам не нужно устанавливать Content-Type: application/octet-stream в общих заголовках.
    # Content-Disposition также обычно не нужен в общих заголовках для multipart,
    # так как имя файла передается в кортеже для параметра `files`.

    upload_target_url = UPLOAD_API_ENDPOINT
    # Основные заголовки (без Content-Type и Content-Disposition, если они мешают multipart)
    http_headers = {
        "Authorization": UPLOAD_API_KEY,
        "User-Agent": DEFAULT_USER_AGENT
        # "Content-Disposition": content_disposition_header, # Обычно не нужен для multipart с httpx.files
    }
    ...
    try:
        for attempt in range(MAX_UPLOAD_RETRIES + 1):
            req_logger.info(f"⏫ Попытка загрузки (multipart) #{attempt + 1}/{MAX_UPLOAD_RETRIES + 1} файла '{original_human_readable_filename}'...")
            response = None
            try:
                # Открываем файл как асинхронный поток для каждой попытки
                async with aiofiles.open(file_path, 'rb') as f_content_stream:
                    # Формируем параметр files для httpx
                    # (имя_поля_формы, (имя_файла, файловый_поток, content_type_файла_если_нужен))
                    # Если API не требует определенного content-type для файла внутри multipart, можно опустить.
                    # mimetypes.guess_type может помочь определить тип, если это важно для API.
                    guessed_mime_type, _ = mimetypes.guess_type(original_human_readable_filename)
                    file_part_content_type = guessed_mime_type or 'application/octet-stream'

                    files_payload = {
                        UPLOAD_API_FORM_FIELD_NAME: (original_human_readable_filename, f_content_stream, file_part_content_type)
                    }
                    # Если API ожидает s3_filename как отдельное поле формы:
                    # data_payload = {"s3_key_name": s3_filename_for_log} # Пример
                    # response = await client_http.post(upload_target_url, headers=http_headers, files=files_payload, data=data_payload)

                    async with httpx.AsyncClient(timeout=UPLOAD_TIMEOUT) as client_http:
                        response = await client_http.post(upload_target_url, headers=http_headers, files=files_payload) 

                    req_logger.info(f"📥 Ответ API [Попытка {attempt + 1}]: Статус {response.status_code}")

                    if response.status_code == 200 or response.status_code == 201:
                        api_result_data = {}
                        try: api_result_data = response.json()
                        except json.JSONDecodeError: req_logger.warning(f"Ответ API не JSON, статус {response.status_code}. Текст: {response.text[:200]}")
                        except Exception: req_logger.exception("Ошибка при обработке JSON ответа")
                        
                        path_from_response = api_result_data.get('url') or api_result_data.get('filePath') or api_result_data.get('link')
                        if not path_from_response and response.headers.get("Location"): path_from_response = response.headers.get("Location")

                        if path_from_response:
                            req_logger.info(f"✅ Успех [Попытка {attempt + 1}]: Путь/URL от API: {path_from_response}")
                            upload_url_result = path_from_response
                        else:
                            req_logger.warning(f"✅ Успех [Попытка {attempt + 1}]: Загрузка успешна (статус {response.status_code}), но URL/путь не получен от API. Используем предложенное S3 имя для логов: {s3_filename_for_log}")
                            upload_url_result = s3_filename_for_log 
            ...
            except MemoryError as e_mem_inner: 
                last_error = f"MemoryError во время попытки #{attempt + 1} загрузки файла '{original_human_readable_filename}': {e_mem_inner}"; req_logger.error(f"🚨 {last_error}", exc_info=True)
                break 
    ...
    except MemoryError as e_mem_outer: 
        last_error = f"MemoryError при подготовке к загрузке файла '{original_human_readable_filename}': {e_mem_outer}"; req_logger.error(f"🚨 {last_error}", exc_info=True)
    except Exception as e_outer: 
        last_error = f"Крит. ошибка при подготовке или во время цикла загрузки '{original_human_readable_filename}': {str(e_outer)}"; req_logger.error(f"🚨 {last_error}", exc_info=True)

    if not upload_url_result and last_error:
        req_logger.error(f"⛔ Загрузка '{original_human_readable_filename}' (S3 имя для логов: '{s3_filename_for_log}') не удалась. Последняя ошибка: {last_error}")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=DOWNLOAD_TIMEOUT, follow_redirects=True, headers=http_headers) as client_http:
            async with client_http.stream("GET", url) as response:
                ...
                unique_s3_filename_for_log = f"{prefix}_{uuid.uuid4().hex}{file_extension}" 
                ...
                req_logger.info(f"📥 Сохранение '{prefix}' (ориг. '{original_human_readable_filename}', S3 имя для логов: '{unique_s3_filename_for_log}') в: {temp_file_path}")
                bytes_downloaded = 0
                ...
                req_logger.info(f"✅ Скачан '{prefix}': '{original_human_readable_filename}' ({bytes_downloaded} байт). Путь: {temp_file_path}. S3 имя для логов: {unique_s3_filename_for_log}")
                return (temp_file_path, unique_s3_filename_for_log, original_human_readable_filename)
...
def _construct_full_url(base_url: Optional[str], relative_path: Optional[str]) -> Optional[str]:
    if not relative_path:
        return None
    if not base_url: 
        logger.warning(f"S3_PUBLIC_BASE_URL не задан, возвращается относительный путь: {relative_path}")
        return relative_path
    
    parsed_relative = urlparse(relative_path)
...
async def process_file_and_send_webhook(
    ...
    main_s3_path_or_url, main_orig_name, lic_s3_path_or_url, lic_orig_name = None, None, None, None
    ...
    try:
        ...
        async with lock_task: 
            ...
            if main_download_result:
                temp_main_path, main_s3_name_for_log, main_orig_name_val = main_download_result
                temp_files_to_clean.append(temp_main_path)
                main_orig_name = main_orig_name_val 
                main_s3_path_or_url = await upload_file_via_api(temp_main_path, task_id, main_s3_name_for_log, main_orig_name_val)
            else:
                err_msg_hook = f"Не удалось скачать основной файл с URL: {main_url_from_bot}"
                tg_err_details = "MainFileDownloadFailed"
                raise Exception(err_msg_hook)
            
            if not main_s3_path_or_url: 
            ...
            if lic_url_from_bot:
                ...
                if lic_download_result:
                    temp_lic_path, lic_s3_name_for_log, lic_orig_name_val = lic_download_result
                    temp_files_to_clean.append(temp_lic_path)
                    lic_orig_name = lic_orig_name_val 
                    lic_s3_path_or_url = await upload_file_via_api(temp_lic_path, task_id, lic_s3_name_for_log, lic_orig_name_val)
                    if not lic_s3_path_or_url:
        ...
        should_send_webhook = bool(err_msg_hook) or bool(main_s3_path_or_url) 
        if should_send_webhook:
            ...
            if task_info_from_db and task_info_from_db.get("webhook_url"):
                ...
                if err_msg_hook: 
                    ...
                    webhook_payload["main_file_url"] = _construct_full_url(S3_PUBLIC_BASE_URL, main_s3_path_or_url)
                ...
                if lic_s3_path_or_url and not err_msg_hook: 
                    webhook_payload["license_file_url"] = _construct_full_url(S3_PUBLIC_BASE_URL, lic_s3_path_or_url)
...
async def get_file_link_synchronous(
    ...
    try:
        ...
        async with lock_obj_sync:
            ...
            main_s3_path_or_url_sync = await upload_file_via_api(main_download_info_sync[0], req_id_sync, main_download_info_sync[1], main_download_info_sync[2])
            if not main_s3_path_or_url_sync:
            ...
            response_data_sync['main_file_url'] = _construct_full_url(S3_PUBLIC_BASE_URL, main_s3_path_or_url_sync)
            response_data_sync['main_file_original_name'] = main_download_info_sync[2]

            if lic_download_info_sync:
                lic_s3_path_or_url_sync = await upload_file_via_api(lic_download_info_sync[0], req_id_sync, lic_download_info_sync[1], lic_download_info_sync[2])
                if lic_s3_path_or_url_sync:
                    response_data_sync['license_file_url'] = _construct_full_url(S3_PUBLIC_BASE_URL, lic_s3_path_or_url_sync)
            ...
            if redirect and response_data_sync.get('main_file_url'):
...
async def health_check(authorized: bool = Depends(verify_api_key)):
    ...
    status_response_payload = {
        ...
        "s3_public_base_url_configured": bool(S3_PUBLIC_BASE_URL)
    }
